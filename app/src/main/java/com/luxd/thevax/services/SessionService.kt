...
class SessionService(context: Context) {
	...
	private val repo: UserRepository = UserRepository(DatabaseHelper(context))
	...
	companion object {
		...
		fun init(context: Context): SessionService {
			return _instance ?: synchronized(this) {
				_instance ?: SessionService(context.applicationContext).also { _instance = it }
			}
		}

		// Returns the singleton instance of SessionService
		fun getInstance(): SessionService {
			return _instance ?: throw IllegalStateException("SessionService non inizializzato!")
		}
	}
