...
class VaccineRepository(db: DatabaseHelper) {
	 ...
	 */
	suspend fun getAll(): List<Vaccine> = withContext(Dispatchers.IO) { vaccineDAO.getAll() }
	 ...
	 */
	suspend fun evaluateVaccinesForUser(user: User, userConditions: List<ClinicalCondition>): List<Pair<Vaccine, String>> {

		return withContext(Dispatchers.IO) {

			// Loads all vaccines + the rules involving the user's therapy & conditions (one query each)
			val vaccines = vaccineDAO.getAll()
			val scheduledVaccineIds = recordDAO.getRecordsForUser(user.id)
				.map { it.vaccineId } //TODO: capire record status (cosa fai se record cancelled?)
			val therapyStatuses = vaccineDAO.getStatusesForTherapy(user.therapyId)
			val conditionStatuses =
				vaccineDAO.getStatusesForConditions(userConditions.map { it.id })
			...
			evaluations.sortedBy { statusOrder[it.second] ?: 3 }
